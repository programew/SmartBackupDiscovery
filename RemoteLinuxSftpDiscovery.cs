...
public sealed record LinuxRemoteTargetSpec(
    string Host,
    int Port,
    IReadOnlyList<string> Roots,
    string? HostKeySha256);

public sealed record LinuxSftpCredential(
    string Username,
    string? Password,
...
public sealed class RemoteLinuxSftpDiscovery
{
        ...
        Action<FileCandidate>? onCandidate = null)
    {
        ...
        if (string.IsNullOrWhiteSpace(credential.Username))
            throw new ArgumentException("Linux SSH username cannot be empty.");
        if (!credential.UsesPrivateKey && credential.Password is null)
            throw new ArgumentException("Linux SSH password is required when no private key is supplied.");
        if (credential.UsesPrivateKey && !File.Exists(Path.GetFullPath(credential.PrivateKeyPath!)))
            throw new FileNotFoundException("SSH private key was not found.", credential.PrivateKeyPath);
        ...
        foreach (LinuxRemoteTargetSpec target in targets)
        {
            ...
            if (expectedFingerprint is null)
                knownHosts.TryGet(target.Host, target.Port, out expectedFingerprint);
            ...
            try
            {
                ...
                foreach (string requestedRoot in target.Roots)
                {
                    ...
                    rootReports.Add(new RemoteLinuxRootAccessReport(
                        sourceRoot,
                        rootScan.Coverage.Exists,
                        rootScan.Coverage.DirectoriesVisited,
                        rootScan.Coverage.FilesSeen,
                        rootScan.Coverage.CandidatesFound,
                        rootScan.Error));
                ...
                if (rootReports.Count > 0 && rootReports.Any(x => !x.Accessible || x.Error is not null))
                    authStatus = rootReports.All(x => !x.Accessible) ? RemoteAuthenticationStatus.Failed : RemoteAuthenticationStatus.Partial;
            }
            catch (Exception ex)
            {
                ...
                {
                    foreach (string root in target.Roots)
                        rootReports.Add(new RemoteLinuxRootAccessReport(RemoteLinuxPath.ToSftpUri(target.Host, target.Port, root), false, 0, 0, 0, ex.Message));
                }
            }
            finally
            {
            ...
            reports.Add(new RemoteLinuxTargetReport(
                target.Host,
                hostName,
                target.Port,
                v4,
        ...
        return new RemoteLinuxScanResult(
            sources.DistinctBy(x => x.Id).ToArray(),
            reports,
    ...
    public static IReadOnlyList<LinuxRemoteTargetSpec> LoadTargets(
        ...
        string? defaultFingerprint)
    {
        ...
        foreach (string host in directHosts)
            raw.Add((ValidateHost(host), null, NormalizeFingerprint(defaultFingerprint)));
        ...
        if (!string.IsNullOrWhiteSpace(hostsFile))
        {
            ...
            foreach (string sourceLine in File.ReadLines(path))
            {
                ...
                if (parts.Length >= 2 && !string.IsNullOrWhiteSpace(parts[1]))
                    roots = parts[1].Split(';', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
                        .Select(RemoteLinuxPath.NormalizeAbsolute).Distinct(StringComparer.Ordinal).ToArray();
                string? fp = parts.Length >= 3 ? NormalizeFingerprint(parts[2]) : null;
                raw.Add((host, roots, fp));
            }
        }

        string[] normalizedDefaults = (defaultRoots.Count > 0 ? defaultRoots : DefaultLinuxRoots)
            .Select(RemoteLinuxPath.NormalizeAbsolute).Distinct(StringComparer.Ordinal).ToArray();

        return raw.GroupBy(x => x.Host, StringComparer.OrdinalIgnoreCase)
            .Select(group =>
            {
    ...
    private static RemoteRootScan ScanRoot(
        SftpClient client,
        LinuxRemoteTargetSpec target,
        string remoteRoot,
        string sourceId,
        TraversalLimits limits,
        PlatformTraversalOptions traversalOptions,
        Dictionary<string, FileCandidate> allCandidates,
        List<string> errors,
        Action<ScanProgress>? progress,
        Action<FileCandidate>? onCandidate)
    {
        string sourceRoot = RemoteLinuxPath.ToSftpUri(target.Host, target.Port, remoteRoot);
        ...
        while (stack.Count > 0)
        {
            if (dirs >= limits.MaxDirectories || files >= limits.MaxFiles)
            {
                completed = false;
                rootErrors++;
                rootError = "Traversal budget reached; remaining remote paths were not scanned.";
                AddError(errors, $"{sourceRoot}: {rootError}");
                break;
            }
            ...
            if (depth > limits.MaxDepth)
            {
                completed = false;
                rootErrors++;
                AddError(errors, $"Maximum depth {limits.MaxDepth} exceeded under {RemoteLinuxPath.ToSftpUri(target.Host, target.Port, dir)}");
                continue;
            }

            if (ShouldSkipRemoteDirectory(dir, traversalOptions, out _))
            {
                policySkipped++; linuxPolicySkipped++;
                continue;
            }
            ...
            try
            {
                entries = client.ListDirectory(dir).Where(x => x.Name is not "." and not "..").ToList();
            }
            catch (Exception ex)
            {
                completed = false;
                rootErrors++;
                if (depth == 0) rootError = ex.Message;
                AddError(errors, $"Cannot enumerate {RemoteLinuxPath.ToSftpUri(target.Host, target.Port, dir)}: {ex.Message}");
                continue;
            }
            ...
            {
                if (entries.Any(x => (x.IsRegularFile || x.IsDirectory) && FileClassifier.IsProjectMarkerName(x.Name)))
                {
                    currentProjectRoot = dir;
                }
                else if (TryInferJvmProjectRoot(dir, remoteRoot, entries, out string? inferred))
                {
                    currentProjectRoot = inferred;
                    projectJvm.Add(inferred!);
                    jvmProjects++;
                }
            }
            ...
            if (currentProjectRoot is not null && !projectSets.ContainsKey(currentProjectRoot))
            {
                ...
                projectSets[currentProjectRoot] = new BackupSet(
                    $"project:{StableId.Hash12(displayRoot)}",
                    "Project",
                    RemoteLinuxPath.GetName(currentProjectRoot),
                    140,
                    BackupPriority.High,
                    new[] { displayRoot },
            ...
            {
                string child;
                try { child = RemoteLinuxPath.Combine(dir, entry.Name); }
                catch { continue; }

                if (entry.IsSymbolicLink)
                {
                    if (entry.IsDirectory) symlinkDirs++; else symlinkFiles++;
                    continue;
                }

                if (entry.IsDirectory)
                {
                    if (ShouldSkipRemoteDirectory(child, traversalOptions, out _))
                    {
                        policySkipped++; linuxPolicySkipped++;
                        continue;
                    }
                    if (currentProjectRoot is not null && FileClassifier.ShouldSkipProjectDirectoryName(entry.Name))
                    {
                        policySkipped++; generatedSkipped++;
                        continue;
                    }
                ...
                if (currentProjectRoot is not null)
                {
                    ...
                    if (!FileClassifier.IsProjectMarkerName(entry.Name))
                        continue;
                }

                LinuxFileHint? remoteHint = LinuxBackupHints.GetHintForRemotePath(child);
                if (LinuxBackupHints.IsMetadataOnlyRemotePath(child) && remoteHint?.MustInclude != true)
                {
                    // Live service data under /var/lib is represented as a logical service set, not thousands of raw file candidates.
                    continue;
                }
                ...
                allCandidates[displayPath] = candidate;
                found++;
        ...
        var volumes = projectFiles.Select(pair => new ProjectVolumeStat(
            RemoteLinuxPath.ToSftpUri(target.Host, target.Port, pair.Key),
            pair.Value,
            projectBytes.GetValueOrDefault(pair.Key),
            projectJvm.Contains(pair.Key))).ToArray();
    ...
    private static bool TryInferJvmProjectRoot(string dir, string scanRoot, IReadOnlyList<ISftpFile> entries, out string? projectRoot)
    {
        ...
        foreach (string suffix in suffixes)
        {
            ...
            string candidate = p[..^suffix.Length];
            if (candidate.Length == 0) candidate = "/";
            projectRoot = RemoteLinuxPath.IsSameOrUnder(candidate, scanRoot) ? candidate : scanRoot;
            return true;
        }

        // A standard repo may have no Maven/Gradle marker left. Detect it as soon as we
        // reach <repo>/src so both main and test trees inherit the same project root.
        if (p.EndsWith("/src", StringComparison.Ordinal) &&
            entries.Any(x => x.IsDirectory && (x.Name.Equals("main", StringComparison.Ordinal) || x.Name.Equals("test", StringComparison.Ordinal))))
        {
            string? parent = RemoteLinuxPath.GetParent(p);
            if (parent is not null && RemoteLinuxPath.IsSameOrUnder(parent, scanRoot))
            {
                projectRoot = parent;
                return true;
            }
    ...
    private static void AddServiceSetIfApplicable(string dir, LinuxRemoteTargetSpec target, string sourceId, Dictionary<string, BackupSet> result)
    {
        foreach (var service in LinuxServiceCatalog.Services)
        {
            ...
            result.TryAdd(id, new BackupSet(
                id,
                "LinuxServiceData",
                service.Name,
                190,
    ...
    private static bool ShouldSkipRemoteDirectory(string path, PlatformTraversalOptions options, out string reason)
    {
        ...
        if (!options.IncludeSystemMounts && SystemVirtualRoots.Any(root => RemoteLinuxPath.IsSameOrUnder(p, root)))
        {
            reason = "system virtual/runtime filesystem";
            return true;
        }
        if (!options.IncludeSystemMounts && ContainerRuntimeRoots.Any(root => RemoteLinuxPath.IsSameOrUnder(p, root)))
        {
            reason = "container runtime/overlay storage";
            return true;
        }
        reason = string.Empty;
        return false;
    ...
    private static SftpClient CreateClient(LinuxRemoteTargetSpec target, LinuxSftpCredential credential, ref PrivateKeyFile? privateKey)
    {
        if (credential.UsesPrivateKey)
        {
            ...
            privateKey = credential.PrivateKeyPassphrase is null
                ? new PrivateKeyFile(keyPath)
                : new PrivateKeyFile(keyPath, credential.PrivateKeyPassphrase);
    ...
    private static MustCopyVolumeSummary CalculateMustCopy(IEnumerable<ProjectVolumeStat> projectVolumes, IEnumerable<FileCandidate> candidates)
    {
        ...
        FileCandidate[] standalone = candidates
            .Where(x => x.MustInclude || x.Priority == BackupPriority.Critical)
            .Where(x => !projectRoots.Any(root => RemoteLinuxPath.IsSameOrUnderSftpUri(x.Path, root)))
            .ToArray();
        long standaloneBytes = SumSaturating(standalone.Select(x => x.Size));
        return new MustCopyVolumeSummary(
            SaturatingAdd(projectFiles, standalone.LongLength),
            SaturatingAdd(projectBytes, standaloneBytes),
            projectFiles,
            projectBytes,
            standalone.LongLength,
            standaloneBytes,
            standalone.Count(x => x.MustInclude),
            SumSaturating(standalone.Where(x => x.MustInclude).Select(x => x.Size)),
            0,
            0,
    ...
    private static IReadOnlyList<BackupSet> BuildRemoteDatabaseSets(IEnumerable<FileCandidate> candidates)
    {
        return candidates.Where(x => x.Category == FileCategory.Database)
            .GroupBy(x => RemoteLinuxPath.TrySplitSftpUri(x.Path, out string host, out int port, out string path)
                ? RemoteLinuxPath.ToSftpUri(host, port, RemoteLinuxPath.GetParent(path) ?? "/")
                : x.Path, StringComparer.Ordinal)
            .Select(group =>
            {
                FileCandidate first = group.First();
    ...
    private static string ValidateHost(string input)
    {
        ...
        if (host.Contains(':') && !IPAddress.TryParse(host, out _))
            throw new ArgumentException($"Linux SSH host '{input}' contains an invalid colon/port form. Use --ssh-port separately.");
        return host.Trim('[', ']');
    }
    ...
    internal static string? NormalizeFingerprint(string? value)
    {
        ...
        if (fp.StartsWith("SHA256:", StringComparison.OrdinalIgnoreCase)) fp = fp[7..];
        fp = fp.TrimEnd('=');
    ...
    {
        long total = 0;
        foreach (long value in values) total = SaturatingAdd(total, Math.Max(0, value));
        return total;
    }
    ...
    private sealed record RemoteRootScan(
        ...
        long LinuxPolicyDirectoriesSkipped,
        IReadOnlyList<ProjectVolumeStat> ProjectVolumes,
        IReadOnlyList<BackupSet> ProjectSets,
        IReadOnlyList<BackupSet> ServiceSets,
        string? Error);
}
...
public sealed class SshKnownHostsStore
{
    ...
    private SshKnownHostsStore(string path, Dictionary<string, string> entries)
    {
        _path = path;
        _entries = entries;
    }
    ...
    public static SshKnownHostsStore Load(string path)
    {
        ...
        if ((File.GetAttributes(full) & FileAttributes.ReparsePoint) != 0)
            throw new IOException("Refusing to read a reparse-point SSH known-hosts file.");
        try
        {
        ...
        {
            throw new InvalidDataException($"SSH known-hosts store is invalid: {full}", ex);
        }
    }

    public bool TryGet(string host, int port, out string? fingerprint)
    {
        if (_entries.TryGetValue(Key(host, port), out string? fp))
        {
            fingerprint = RemoteLinuxSftpDiscovery.NormalizeFingerprint(fp);
            return true;
        }
        fingerprint = null;
        return false;
    ...
    public void Save()
    {
        ...
        if (File.Exists(_path) && (File.GetAttributes(_path) & FileAttributes.ReparsePoint) != 0)
            throw new IOException("Refusing to overwrite a reparse-point SSH known-hosts file.");
